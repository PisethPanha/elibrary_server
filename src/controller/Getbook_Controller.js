...
function AddBookController(req, res) {
    ...
    db.connection.query(queryLastId, (err, result) => {
        ...
        const query = `INSERT INTO book ( Title, describetion, autor, publisher, publish_date, img, img_content1, img_content2, img_content3,  link_download, language, type ) 
        VALUES ( '${title}','${description}', '${author}', '${publisher}', '${date}', '${ID + 1}${image1}', '${ID+1 + image1}', '${ID+1 + image2}', '${ID+1 + image3}', '${link}', '${language}', '${type}' )`
        db.connection.query(query, (err, result) => {
